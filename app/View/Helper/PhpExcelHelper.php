...
class PhpExcelHelper extends AppHelper {
    ...
    public function addSheet($name) {
     ...
     public function getColonneDimension($dimension) {
        $this->_xls->getActiveSheet()->getColumnDimensionByColumn(3)->setWidth(20);
     ...
     * @return $this for method chaining
     */
    public function addTableRow($data,$merge,$params = array()) {
        ...
        foreach ($data as $d)
            ...
            /*if($d != 0){
                $this->_xls->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($offset++).$this->_row)->applyFromArray(array( 'fill' => 
        ...
        /*if($tel_format == 1){
            $this->_xls->getActiveSheet()->getStyle('E'.$this->_row) -> getNumberFormat() -> setFormatCode('0000000000');
        }*/
    ...
    public function formater($col) {      
        $this->_xls->getActiveSheet()->getStyle($col) -> getNumberFormat() -> setFormatCode('000');
        return $this;
    }

    public function border_cell() {  
        ...
        $bordersarray=array(
            ...
            $this->_xls->getActiveSheet()->getStyle('A1:E1')->applyFromArray($bordersarray);        
            return $this;
    }
    ...
    public function font($size) {  
            ...
            $this->_xls->getActiveSheet()->getStyle('A1')->getFont()->setSize($size);      
            return $this;
    }


    public function formaterDossard($col) {      
        $this->_xls->getActiveSheet()->getStyle($col) -> getNumberFormat() -> setFormatCode('0000');
        return $this;
    }

    public function formaterTel($col) {      
        $this->_xls->getActiveSheet()->getStyle($col) -> getNumberFormat() -> setFormatCode('00 00 00 00 00');
        return $this;
    }
    ...
    public function cellsToMergeByRow($start = NULL, $end = NULL, $row = NULL){
        $merge = 'A1:A1';
        if($start && $end && $row){
     ...
     public function cellsToMergeByCol($start = NULL, $end = NULL, $col = NULL){
        $merge = 'A1:A1';
        if($start && $end && $col){
            $col = PHPExcel_Cell::stringFromColumnIndex($col);
            $merge = "$col{$start}:$col{$end}";
