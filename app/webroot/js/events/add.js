...
function(){
	...
	$("#btnLibAuto").click(function(){	
		...
		if($('#type option:selected').text() == 'Championnat - Phase(s) finale(s)' || $('#type option:selected').text() == 'Championnat - Phase(s) qualificative(s)'){
			typeAuto = 'Championnat de France';
		} else{
			typeAuto = $('#type option:selected').text();
		}

		if( $("#multisport0").is(':checked')){
			if( $("#pays").val() == 75){
				$("#nameAuto").val(typeAuto + ' ' + $("#type_complement").val() + ' - ' + $("#activite").val() + ' - ' + $("#villeFR").val().toUpperCase() + ' (' + arrayPays[1] + ' - ' + $("#year").val());	
			} else {
					
				$("#nameAuto").val(typeAuto + ' ' + $("#type_complement").val() + ' - ' + $("#activite").val() + ' - ' + $("#villeETR").val().toUpperCase()+ ' (' + arrayPays[1] + ' - ' + $("#year").val());	
			}
		} 
		if( $("#multisport1").is(':checked') ){
			if( $("#pays").val() == 75){
				$("#nameAuto").val(typeAuto + ' ' + $("#type_complement").val() + ' - ' + $("#villeFR").val().toUpperCase() + ' (' + arrayPays[1] + ' - ' + $("#year").val());	
			} else {
				$("#nameAuto").val(typeAuto + ' ' + $("#type_complement").val() + ' - ' + $("#villeETR").val().toUpperCase()+ ' (' + arrayPays[1] + ' - ' + $("#year").val());	
			}
		}
	});
